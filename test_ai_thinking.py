...
async def analyze_dialogue_decision(conversation_history, budget="$$"):
    ...
    try:
        ...
        # Системный промпт из main.py (улучшенная версия с учетом полного контекста)
...
Отвечай естественно на русском языке. Обращайся на "Вы"."""
        ...
        response = client.chat.completions.create(
            model=openai_model,
            messages=messages,
            temperature=0.7,  # Немного снижаем для более стабильных результатов
            max_tokens=150
        )
        
        ai_response = response.choices[0].message.content.strip()
        
        # Проверяем, решил ли AI, что пора искать в базе
    ...
    for turn in scenario['turns']:
        ...
        if turn['type'] == 'user':
            ...
            if analysis['should_search']:
                ...
                # Проверяем, не должен ли был AI уже начать поиск
                expected_decision = turn.get('expected_decision', 'continue')
                if expected_decision == 'search':
            ...
            # Заранее определенный ответ бота (для контроля сценария)
            conversation_history.append({"role": "assistant", "content": turn['message']})
            print(f"🤖 Бот: {turn['message']}")
        
        step += 1
    
    # Если дошли до конца без поиска, проверяем финальное ожидание
...
async def run_realistic_tests():
    """Запускает реалистичные тесты с развивающимися диалогами"""
    ...
    scenarios = [
        ...
        # === ДОПОЛНИТЕЛЬНЫЕ РАЗНООБРАЗНЫЕ СЦЕНАРИИ ===
        {
            "name": "Завтрак/бранч",
            "budget": "$",
            "turns": [
        ...
        {
            "name": "Ресторан с детской площадкой",
            "budget": "$$",
            "turns": [
                {"type": "user", "message": "ищу семейный ресторан", "expected_decision": "continue"},
                {"type": "bot", "message": "Какие требования к детской зоне?"},
                {"type": "user", "message": "чтобы дети могли играть, а мы спокойно поесть", "expected_decision": "search"}
            ]
        },
        {
            "name": "Винный ужин",
        ...
        },
        {
            ...
            "turns": [
                ...
                {"type": "user", "message": "настоящую острую, как едят местные", "expected_decision": "search"}
            ]
        },
        {
        ...
        {
            "name": "Ресторан с завтраком",
            "budget": "$",
            "turns": [
                {"type": "user", "message": "рано утром, нужен завтрак", "expected_decision": "continue"},
                {"type": "bot", "message": "Европейский или азиатский завтрак?"},
                {"type": "user", "message": "европейский, яйца и кофе", "expected_decision": "search"}
            ]
        },
        {
            "name": "Ресторан с суши-мастером",
            "budget": "$$$",
            "turns": [
                {"type": "user", "message": "хочу настоящие суши от японского мастера", "expected_decision": "continue"},
                {"type": "bot", "message": "Омакасе или а-ля карт?"},
                {"type": "user", "message": "омакасе, доверюсь шефу", "expected_decision": "search"}
            ]
        },
        {
            "name": "Ресторан с барной стойкой",
            "budget": "$$",
            "turns": [
                {"type": "user", "message": "хочу сидеть за барной стойкой", "expected_decision": "continue"},
                {"type": "bot", "message": "Какую кухню и напитки?"},
                {"type": "user", "message": "тапас и вино", "expected_decision": "search"}
            ]
        },
        {
            "name": "Ресторан с шеф-поваром",
            "budget": "$$$$",
            "turns": [
                {"type": "user", "message": "хочу встретиться с шеф-поваром", "expected_decision": "continue"},
                {"type": "bot", "message": "Какая кухня интересует?"},
                {"type": "user", "message": "авторская, современная интерпретация тайской", "expected_decision": "search"}
            ]
        }
    ]
...
def test_ai_thinking():
    """Test AI thinking logic for dialogue decisions"""
